...
### 🔴 高嚴重性 (High Severity)
1. **專案結構混亂、方法關係不明確**：專案主方法（`review-crawler-agent/`）與舊版方法（`analysis/`、`site/`）檔案散落在根目錄，使整體結構難以維護。建議依下表進行檔案歸類與遷移，同時將 `review-crawler-agent/` 改名為 `crawler/`、`analysis/` 更名為 `experiments/` 以凸顯其試驗性質；主方法依賴統一放在根目錄的 `requirements.txt`，試驗依賴則留在各試驗資料夾中（如 `experiments/requirements.txt`），保持根目錄乾淨：
...
2. **數據儲存與傳輸使用 Markdown 格式不穩定**：目前爬蟲將原始評論儲存為 `*_App_Reviews.md` 內的 Markdown 表格，解析時極易因為用戶評論中的換行、管道符號（`|`）或 Markdown 特殊字元導致解析破裂。建議改為使用標準的 JSON 檔案儲存原始數據，並更新後續的同步與分析程式。
3. **流程問題**：LLM整理的md摘要並沒有透過簡單的流程匯到web端。建議：未來重構時，讓 LLM 直接輸出結構化的 JSON 摘要檔，打通從爬蟲、LLM 分析到網頁顯示的完整自動化流程。
...
│   ├── 📁 comments/                  # 原始評論（按 App、按月分片，滿月封存）
│   │   ├── 📄 2026-05.json           # 封存月份（唯讀，不再修改）
│   │   ├── 📄 2026-06.json
│   │   └── 📄 2026-07.json           # 當月持續寫入
